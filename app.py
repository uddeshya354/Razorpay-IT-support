...
#     # --- NEW: Daily Transactions Dataset for Bar Chart ---
#     daily_txns = pd.DataFrame()
#     if 'date_only' in df_filtered.columns:
#         daily_txns = df_filtered.groupby('date_only').agg(
#             Completed_Transactions=('Is_Completed', 'sum'),
#             Initiated_Transactions=('Is_Initiated', 'sum')
#         ).reset_index()
#         # Ensure it's a datetime object for better plotting on X-axis
#         daily_txns['date_only'] = pd.to_datetime(daily_txns['date_only'])
#         # Sort by date
#         daily_txns = daily_txns.sort_values('date_only')
...
#         loc_rev['Refunded_Revenue'] = loc_rev['Refunded_Revenue'].round(2)
...
#             # Calculate overall size distribution for our new Bar Chart
#             size_map = {'Medium': 'M', 'Large': 'L', 'Extra Large': 'XL'}
#             overall_sizes = df_completed['locker size'].map(size_map).fillna(df_completed['locker size']).value_counts(normalize=True).reset_index()
#             overall_sizes.columns = ['Locker Size', 'Percentage (%)']
#             overall_sizes['Percentage (%)'] = (overall_sizes['Percentage (%)'] * 100).round(2)
            
#             # Continue with existing location-specific size stats
...
#     return loc_rev, city_rev, time_dist, user_df, repeat_users, loc_repeat_stats, daily_txns, overall_sizes
...
#             # UNPACK NEW METRICS
#             loc_rev, city_rev, time_dist, user_df, repeat_users, loc_repeat_stats, daily_txns, overall_sizes = generate_backend_analytics(backend_df, refund_df)
...
#             with c_chart1:
#                 st.write("### 📈 Daily Transactions (Volume Over Time)")
#                 if not daily_txns.empty:
#                     # Sets date to index so Streamlit natively plots it as the X-axis
#                     plot_data = daily_txns.set_index('date_only')[['Completed_Transactions', 'Initiated_Transactions']]
#                     st.bar_chart(plot_data)
#                 else:
#                     st.info("No timeline data available.")
                    
#             with c_chart2:
#                 st.write("### 📦 Overall Locker Size Utilization")
#                 if not overall_sizes.empty:
#                     size_plot_data = overall_sizes.set_index('Locker Size')
#                     st.bar_chart(size_plot_data)
#                 else:
#                     st.info("No locker size data available.")
...
def generate_backend_analytics(raw_backend_df, raw_refund_df=None):
    ...
    if 'date created' in df_filtered.columns:
        ...
        # --- NEW: Utilization Heatmap Matrix ---
        completed_only = df_filtered[df_filtered['Is_Completed'] == 1]
        if not completed_only.empty:
            cats = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
            completed_only['day_of_week'] = pd.Categorical(completed_only['day_of_week'], categories=cats, ordered=True)
            
            # Crosstab creates a beautiful pivot table showing volume per day per time block
            utilization_matrix = pd.crosstab(completed_only['day_of_week'], completed_only['time_of_day'])
    ...
    return loc_rev, city_rev, time_dist, user_df, repeat_users, loc_repeat_stats, daily_txns, overall_sizes, seasonality_df, utilization_matrix
...
with tab1:
    ...
    if backend_files:
        try:
            ...
            # UNPACK ALL 10 METRICS
            loc_rev, city_rev, time_dist, user_df, repeat_users, loc_repeat_stats, daily_txns, overall_sizes, seasonality_df, utilization_matrix = generate_backend_analytics(backend_df, refund_df)
            st.success(f"✅ {len(backend_files)} Backend file(s) combined & Analytics generated!")
        except Exception as e:
            st.error(f"Error processing Backend Data: {e}")
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            if backend_files:
                ...
                if not daily_txns.empty: 
                    ...
                    export_txns['date_only'] = export_txns['date_only'].astype(str)
                    export_txns.to_excel(writer, index=False, sheet_name='Daily_Transactions')
                if not seasonality_df.empty: seasonality_df.to_excel(writer, index=False, sheet_name='Monthly_Seasonality')
                if not utilization_matrix.empty: utilization_matrix.to_excel(writer, sheet_name='Utilization_Heatmap')
                if not overall_sizes.empty: overall_sizes.to_excel(writer, index=False, sheet_name='Overall_Locker_Sizes')
                if not time_dist.empty: time_dist.to_excel(writer, index=False, sheet_name='Time_of_Day_Stats')
                if not loc_repeat_stats.empty: loc_repeat_stats.to_excel(writer, index=False, sheet_name='Loyalty_Stats')
                if not user_df.empty:
                    user_df.to_excel(writer, index=False, sheet_name='All_User_Analytics')
...
with tab2:
    if not backend_files:
        ...
        if loc_rev.empty and city_rev.empty:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.write("### 🔥 Demand Heatmap (Day vs Time)")
                st.caption("Darker blue indicates higher transaction volume.")
                if not utilization_matrix.empty:
                    # Renders a beautiful color-scaled heatmap in Streamlit natively!
                    st.dataframe(utilization_matrix.style.background_gradient(cmap='Blues', axis=None), use_container_width=True)
                else:
                    st.info("Not enough data to map utilization.")
